...
// Users can search products, filter by category, and sort results.
...
// These arrays are used to build the SQL query safely.
$where = ['is_active = 1'];
...
// Filter by category if a valid category was selected.
...
// Search by product name, origin, or description.
if ($search !== '') {
...
// Convert the selected sort option into a safe SQL order.
// The user input is never directly added to the query.
$orderBy = match ($sort) {
...
<h1>
<?= $category ? h(ucfirst($category)) : 'Shop All Products' ?>
</h1>


<p>
<?= (int) $resultCount ?> product<?= $resultCount === 1 ? '' : 's' ?> found
</p>
...
<!-- GET method keeps filters visible in the URL so they can be shared -->
...
<input type="text"
       ...
       name="q"
       value="<?= h($search) ?>"
       placeholder="e.g. Ethiopian, jasmine...">
...
<option value="coffee" <?= $category === 'coffee' ? 'selected' : '' ?>>
Coffee
</option>


<option value="tea" <?= $category === 'tea' ? 'selected' : '' ?>>
Tea
</option>
...
<a class="product-card card-link"
   href="product.php?slug=<?= h($p['slug']) ?>">
...
// Close database statement after using it.
